...
SELECT c.id, c.code, c.name, c.precision, c.is_fiat, c.blockchain, c.contract_address, c.withdrawal_min_balance, c.has_balance, c.status, c.sort_order, c.min_confirmation, c.created_at, c.updated_at, c.is_stablecoin, c.currency_label, c.token_label, c.is_native, c.is_new_store_default, c.order_idx, ass.service_slug, ass.currency_id, ass.asset_identity, ass.blockchain_name
FROM aml_supported_assets ass
         INNER JOIN currencies c on c.id = ass.currency_id
WHERE currency_id = $1
  AND service_slug = $2
