...
SELECT wfpw.id, wfpw.currency_id, wfpw.store_id, wfpw.transfer_id, wfpw.address_from, wfpw.address_to, wfpw.amount, wfpw.amount_usd, wfpw.created_at, wfpw.updated_at, wfpw.request_id, wfpw.blocked_by_processing_error,
       coalesce(t.kind, ''),
       coalesce(t.stage, ''),
       coalesce(t.status, '')
...
func (q *Queries) FindByTransferID(ctx context.Context, id uuid.UUID) (*FindByTransferIDRow, error) {
	...
	err := row.Scan(
		...
		&i.WithdrawalFromProcessingWallet.BlockedByProcessingError,
		&i.Kind,
		&i.Stage,
		&i.Status,
	)
...
SELECT id, currency_id, store_id, transfer_id, address_from, address_to, amount, amount_usd, created_at, updated_at, request_id, blocked_by_processing_error
FROM withdrawal_from_processing_wallets
WHERE id = $1
  AND store_id = $2
`
...
const setBlockedByProcessingError = `-- name: SetBlockedByProcessingError :exec
UPDATE withdrawal_from_processing_wallets
SET blocked_by_processing_error = $1
WHERE id = $2
`

type SetBlockedByProcessingErrorParams struct {
	BlockedByProcessingError bool      `db:"blocked_by_processing_error" json:"blocked_by_processing_error"`
	ID                       uuid.UUID `db:"id" json:"id"`
}

func (q *Queries) SetBlockedByProcessingError(ctx context.Context, arg SetBlockedByProcessingErrorParams) error {
	_, err := q.db.Exec(ctx, setBlockedByProcessingError, arg.BlockedByProcessingError, arg.ID)
	return err
}
