...
func (q *Queries) GetFullDataByID(ctx context.Context, id uuid.UUID) (*GetFullDataByIDRow, error) {
	...
	err := row.Scan(
		...
		&i.Store.Description,
		&i.Store.VerificationComment,
	)
	return &i, err
}

const getWalletWithStore = `-- name: GetWalletWithStore :many
SELECT w.id as wallet_id, w.store_external_id, wa.address, wa.currency_id, s.id, s.user_id, s.name, s.site, s.currency_id, s.rate_source, s.return_url, s.success_url, s.rate_scale, s.status, s.minimal_payment, s.created_at, s.updated_at, s.deleted_at, s.public_payment_form_enabled, s.verification_status, s.verified_at, s.verified_by, s.rejection_reason, s.description, s.verification_comment
FROM wallets w
