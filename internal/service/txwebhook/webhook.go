...
func (s *Service) handleDepositReceived(ev event.IEvent) error {
	...
	return s.processWebhooksByTransactionEvent(ev, convertedEv.EventType(), payload)
}
...
func (s *Service) amlGate(ctx context.Context, ev transactions.TransactionEvent, settings *models.UserAmlSetting) (*models.AmlCheck, bool) {
	...
	blocked := s.amlService.ApplyVerdict(ctx, aml.ApplyVerdictDTO{
		Check:         amlCheck,
		UserID:        ev.GetStore().UserID,
		StoreID:       ev.GetStore().ID,
		TransactionID: ev.GetTx().GetID(),
		WalletID:      ev.GetTx().GetWalletID(),
		ToAddress:     ev.GetTx().GetToAddress(),
		Signals:       signals,
		Rules:         rules,
	})
...
func (s *Service) SendWebhookManual(ctx context.Context, txID, userID uuid.UUID) error {
	...
	hasAccess, err := s.storeService.CheckUserHasAccess(ctx, userID, tx.GetStoreID())
	if err != nil {
		return fmt.Errorf("check user access: %w", err)
	}
	if !hasAccess {
		return store.ErrUserHasNoAccess
	}
...
func (s *Service) handleAMLCheckCompleted(ev event.IEvent) error {
	...
	str, err := s.storage.Stores().GetByID(ctx, tx.StoreID.UUID)
	if err != nil {
		return fmt.Errorf("fetch store: %w", err)
	}

	amlSettings, err := s.storage.UserAmlSettings().GetByUserID(ctx, str.UserID)
	if err != nil {
		return fmt.Errorf("fetch aml settings: %w", err)
	}
	if amlSettings.ProviderSlug == nil {
		return fmt.Errorf("aml provider not configured for user %s", str.UserID)
	}
	...
	if blocked {
		s.log.Warnw("AML check triggered a blocking rule", "store_id", str.ID, "tx_id", tx.ID, "score", completedEv.Check.Score)
		return s.sendAMLBlockedWebhook(ctx, tx, str.ID, *curr, storeExternalID, &completedEv.Check, nil)
	}
	...
	return s.sendWebhookForTx(ctx, tx.ID, str.ID, models.WebhookEventPaymentReceived, payload, nil)
}
